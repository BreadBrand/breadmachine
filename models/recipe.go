...
func (r *Recipe) CalculateBakerPercentages() {
	...
	for _, ing := range r.Ingredients {
		if ing.Phase == PhaseDough && strings.Contains(strings.ToLower(ing.IngredientName), "flour") {
	...
	for i := range r.Ingredients {
		if totalFlour > 0 {
			r.Ingredients[i].BakerPercentage = (r.Ingredients[i].Quantity / totalFlour) * 100
		} else {
